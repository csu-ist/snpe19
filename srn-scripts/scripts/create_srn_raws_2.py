...
#    mean_raw = __create_mean_raw(img_raw, mean_rgb)
    ...
    snpe_raw_filename += '_256x256.raw'
...
def convert_img(src,dest,height, width):
    ...
    for jpgs in os.listdir(src):
        if(True):
        #for jpgs in files:
            src_image=os.path.join(src, jpgs)
            if('.jpg' in src_image):
                print(src_image)
		if(True): # src_image=="/opt/SNPE/snpe-1.19.2/models/srn/data/chairs.jpg"):
