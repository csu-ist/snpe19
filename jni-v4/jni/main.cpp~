...
int main(int argc, char** argv)
{
    ...
    size_t img_width = 500;
    size_t img_height = 500;
...
#endif
    snpe = setBuilderOptions(container, runtime, udlBundle, useUserSuppliedBuffers, platformConfig, img_width, img_height);
    ...
    std::cerr << "Now tensorShape: " << tensorShape.getDimensions()[0]<< ", " << tensorShape.getDimensions()[1]<< ", " << tensorShape.getDimensions()[2]<< ", " << tensorShape.getDimensions()[3]<<std::endl;
#ifdef ANDROID
    if (userBufferSourceType == GLBUFFER)
    {
    ...
    if(bufferType == USERBUFFER) {
        ...
        std::cerr<< "createOutputBufferMap: "<<std::endl;
        ...
        if (userBufferSourceType == CPUBUFFER)
        {
            ...
            std::cerr<< "createInputBufferMap: "<<std::endl;
