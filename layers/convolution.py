...
class ConvolutionLayerBuilder(LayerBuilder):
	...
	if padding.decode() == "VALID": #workaround, hardcoded, we assume the padding is SAME 
 	    print input_size,output_size,strides,padding,filter_dims,dilation 
 	    filter_h = 1 + (filter_dims[0] - 1) * dilation[0] 
 	    filter_w = 1 + (filter_dims[1] - 1) * dilation[1] 
 	    pad_y = max(((input_size[0] - 1) * strides[0] + filter_h - input_size[0]), 0) 
 	    pad_x = max(((input_size[1] - 1) * strides[1] + filter_w - input_size[1]), 0) 
 	    pad_y //= 2 
 	    pad_x //= 2 
